function check() {
    ...
    const matchesregex = /^[a-zA-Z0-9!$%^&*()-_=+]+$/.test(document.getElementById("pswd_to_check").value);
    if (25>document.getElementById("pswd_to_check").value.length) { correctlength = document.getElementById("pswd_to_check").value.length > 7; } else { correctlength = false; }
    const ucl = /^(?=.*[A-Z])/.test(document.getElementById("pswd_to_check").value);
    const lcl = /^(?=.*[a-z])/.test(document.getElementById("pswd_to_check").value);
    const digit = /^(?=.*[0-9])/.test(document.getElementById("pswd_to_check").value);
    const allowedsymbols = /^(?=.*[!$%^&*()_=+-])/.test(document.getElementById("pswd_to_check").value);
    const onlyLetters = /^[a-zA-Z]+$/.test(document.getElementById("pswd_to_check").value);
    const onlyDigits = /^[0-9]+$/.test(document.getElementById("pswd_to_check").value);
    const onlySymbols = /^[!$%^&*()_=+-]+$/.test(document.getElementById("pswd_to_check").value);
    ...
    extraifall = !!(matchesregex && correctlength && ucl && lcl && digit && allowedsymbols);
