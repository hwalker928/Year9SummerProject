function generate() {
  ...
  while (weak) {
    ...
    var matchesregex = /^[a-zA-Z0-9!$%^&*()-_=+]+$/.test(
      document.getElementById("final_password").value
    );
    var ucl = /^(?=.*[A-Z])/.test(
      document.getElementById("final_password").value
    );
    ...
    var allowedsymbols = /^(?=.*[!$%^&*()_=+-])/.test(
      document.getElementById("final_password").value
    );
    var onlyLetters = /^[a-zA-Z]+$/.test(
      document.getElementById("final_password").value
    );
    var onlyDigits = /^[0-9]+$/.test(
      document.getElementById("final_password").value
    );
    var onlySymbols = /^[!$%^&*()_=+-]+$/.test(
      document.getElementById("final_password").value
    );
